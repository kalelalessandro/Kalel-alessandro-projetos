...
class SpeechApp(ctk.CTk):
    ...
    def toggle_listening(self):
        if not self.listening:
            ...
            self.listening = False
            self.listen_button.configure(text="Iniciar Transcrição")
    ...
    def listen_microphone(self):
        ...
        while self.listening:
            ...
            if self.recognizer.AcceptWaveform(data):
                ...
                if text:
                    self.textbox.insert("end", text.strip().capitalize() + ". ")
                    self.textbox.see("end")
        stream.stop_stream()
        stream.close()

    def abrir_libras(self):
        texto = self.textbox.get("1.0", "end").strip()
        if not texto:
            return

        texto_url = urllib.parse.quote(texto)
        url = f"https://www.handtalk.me/tradutor/?text={texto_url}"
        webbrowser.open(url)
